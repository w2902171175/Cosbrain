...
本文档详细描述了学生项目协作平台的数据库表结构，共包含 **32个数据表**。
...
**约束**: UniqueConstraint(room_id, member_id) - 确保一个用户在一个聊天室中只有一条成员记录
...
**约束**: 索引确保一个用户在一个聊天室中最多只有一个'pending'状态的申请
...
| name | String | NOT NULL, INDEX | 知识库名称 |
| description | Text | NULLABLE | 知识库描述 |
| access_type | String | NULLABLE | 访问类型 |
| created_at | DateTime | DEFAULT now() | 创建时间 |
| updated_at | DateTime | ON UPDATE now() | 更新时间 |
...
### 4.4 knowledge_document_chunks (知识文档分块表)

存储文档RAG分块信息。

| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | Integer | PRIMARY KEY, INDEX | 分块唯一标识符 |
| document_id | Integer | FOREIGN KEY(knowledge_documents.id), NOT NULL | 所属文档ID |
| owner_id | Integer | FOREIGN KEY(students.id), NOT NULL | 拥有者ID |
| kb_id | Integer | FOREIGN KEY(knowledge_bases.id), NOT NULL | 所属知识库ID |
...
| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | Integer | Primary Key, Auto Increment | 材料唯一标识 |
| course_id | Integer | Foreign Key(courses.id), Not Null, Index | 所属课程ID |
...
| content | Text | Nullable | 材料的文本内容或简要描述 |
...
**约束**: 
- UniqueConstraint(course_id, title) - 确保同一课程下材料标题唯一
- UniqueConstraint(file_path) - 确保文件路径唯一
...
| reason | String | Nullable | 积分变动理由描述 |
...
4. **知识库相关关系**
   ...
   - `knowledge_documents.owner_id` → `students.id`
...
1. **聊天室成员唯一性**: 一个用户在一个聊天室中只能有一条成员记录
2. **成就唯一性**: 一个用户不会重复获得同一个成就
3. **待处理申请唯一性**: 一个用户在一个聊天室中最多只有一个pending状态的加入申请
4. **TTS配置名称唯一性**: 同一个用户下TTS配置名称唯一
5. **课程材料标题唯一性**: 同一课程下材料标题唯一
6. **文件路径唯一性**: 防止重复上传同一个文件
