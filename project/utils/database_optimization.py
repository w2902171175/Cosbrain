# project/utils/database_optimization.py
...
from project.models import ForumTopic, ForumComment, ForumLike, Student, UserFollow
    ...
    def get_hot_topics(db: Session, limit: int = 20, time_range_hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            hot_topics_query = db.query(
                ...
                Student.name.label('author_name'),
                Student.avatar_url.label('author_avatar')
            ).join(
                Student, ForumTopic.owner_id == Student.id
            ).filter(
    ...
    def get_topic_comments_paginated(db: Session, topic_id: int, page: int = 1, 
        ...
        try:
            ...
            comments_query = db.query(
                ...
                Student, ForumComment.owner_id == Student.id
            ).filter(
                ForumComment.topic_id == topic_id
            ).order_by(
    ...
    def get_topic_statistics(db: Session, topic_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_likes = db.query(
                ...
                Student, ForumLike.owner_id == Student.id
            ).filter(
...
class CacheRefreshScheduler:
    ...
    @staticmethod
    def refresh_user_stats_cache(db: Session, user_id: int):
        """刷新用户统计缓存"""
        try:
            # 清除用户相关缓存
            ...
            logger.info(f"用户{user_id}统计缓存刷新完成")
        except Exception as e:
            logger.error(f"刷新用户统计缓存失败: {e}")
