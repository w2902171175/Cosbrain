...
from project.models import User, Project, UserCourse, ForumTopic, ForumComment, ForumLike, ChatMessage, PointTransaction, Achievement, UserAchievement
...
def check_admin_permission(db: Session, current_user_id: int) -> User:
    """
    检查用户是否具有管理员权限
...
def check_resource_permission(resource, current_user_id: int, admin_user: User = None, 
...
def check_project_permission(db: Session, project_id: int, current_user_id: int, 
                           current_user: User = None, required_permissions: List[str] = None,
...
def validate_file_type(file_content_type: str, allowed_types: List[str] = None, 
                      forbidden_types: List[str] = None, error_message: str = None):
    """
    验证文件类型的通用函数
    ...
    Args:
        ...
        forbidden_types: 禁止的MIME类型列表（如 ['image/', 'video/']）
        error_message: 自定义错误信息
    
    Raises:
        HTTPException: 如果文件类型不符合要求
    """
    # 默认允许的项目文件类型
    ...
    if forbidden_types:
        for forbidden_type in forbidden_types:
            if file_content_type.startswith(forbidden_type):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=error_message or f"不支持的文件类型: {file_content_type}"
                )
    ...
    if file_content_type not in allowed_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=error_message or f"不支持的文件类型: {file_content_type}。仅支持常见文档、文本和代码文件。"
        )
